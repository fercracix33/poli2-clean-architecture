 ...
 */
export async function getUserOrganizationsFromDB(userId: string): Promise<Organization[]> {
  try {
    const { data, error } = await supabase
      .from('organizations')
      .select(`
 ...
 */
export async function getOrganizationCreatorFromDB(organizationId: string): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('organizations')
      .select('created_by')
